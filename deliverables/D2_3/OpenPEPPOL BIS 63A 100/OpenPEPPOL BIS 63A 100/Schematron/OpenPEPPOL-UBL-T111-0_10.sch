  ...
  <ns prefix="xs" uri="http://www.w3.org/2001/XMLSchema"/>
	...
	<pattern id="business_rules">
		...
		<rule context="cbc:CustomizationID">
			<assert id="OP-T111-R002" 
					...
					flag="fatal">[OP-T111-R002]- An Invoice Response SHALL have the customization identifier "urn:www.peppol.eu:transaction:biitrns111:ver1.0".</assert>
		</rule>

		<!-- Status: Done
		[OP-0111-R003]-An Invoice Response SHALL contain the date of issue.
		...
		<rule context="ubl:ApplicationResponse">
			...
			<assert id="OP-T111-R004" 
					...
					flag="fatal">[OP-T111-R004]-The Endpoint ID for party sending Invoice Response SHALL be specified.</assert>
			...
			<assert id="OP-T111-R005" 
					...
					flag="fatal">[OP-T111-R005]-The party sending Invoice Response SHALL be specified.</assert>
			...
			<assert id="OP-T111-R006" 
					...
					flag="fatal">[OP-T111-R006]-The Endpoint ID for receiving sending Invoice Response SHALL be specified.</assert>
			...
			<assert id="OP-T111-R007" 
					...
					flag="fatal">[OP-T111-R007]-The party receiving Invoice Response SHALL be specified.</assert>
		</rule>
		...
		<rule context="cac:DocumentResponse/cac:DocumentReference">
			<assert id="OP-T111-R008"
					flag="fatal" 				
					test="count(cbc:ID)=1">[OP-T111-R008]-An Invoice Response SHALL contain a document reference pointing towards the business message that the response relates to.</assert>
		</rule>
		...
		<rule context="cac:DocumentResponse/cac:Response">
			<assert id="OP-0111-R009" 
				test="( ( not(contains(normalize-space(cbc:ResponseCode),' ')) and contains( ' CA UQ RE ',concat(' ',normalize-space(cbc:ResponseCode),' ') ) ) ) and count(cac:Status/cbc:StatusReasonCode)>=1 or (not(contains( ' CA UQ RE ',concat(' ',normalize-space(cbc:ResponseCode),' ') ) ) )" 
					flag="fatal" >[OP-0111-R009]-IF status code is one of: CA, UQ or RE then there SHALL be at a clarification code in Invoice Response.</assert>
		</rule>
	...
	<pattern id="codelist_rules">
		...
		<rule context="cac:DocumentResponse/cac:Response/cac:Status/cbc:StatusReasonCode">
			...
			<assert id="CL-T111-R013"
					test="(( normalize-space(.) = 'OTH' and (../cbc:StatusReason != ' ') ) or normalize-space(.) != 'OTH') " 
					flag="warning">[CL-T111-R013]-If Clarification code is OTH then Clarification reason SHOULD be provided.</assert>
